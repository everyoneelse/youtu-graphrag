...
DEFAULT_SEMANTIC_DEDUP_PROMPT = (
    ...
    "Candidate tails:\n"
    "{candidates}\n\n"
    "TASK: This is a TWO-STEP process:\n"
    "STEP 1: Identify which tails are COREFERENT (refer to the exact same entity/concept) and group them.\n"
    "STEP 2: For each group, select the most informative expression as the representative.\n\n"
    "=" * 80 + "\n"
    "STEP 1: EQUIVALENCE GROUPING (Determine which tails should be merged)\n"
    "=" * 80 + "\n\n"
    "CRITICAL: RELATION-DEPENDENT DEDUPLICATION\n"
    "The deduplication behavior depends on the relation type:\n\n"
    "DO NOT DEDUPLICATE if the relation explicitly collects multiple expressions:\n"
    "• Alias/Alternative name relations: 'has_alias', 'also_known_as', 'alternative_name'\n"
    "  Example: (MRI, has_alias, 'magnetic resonance imaging') + (MRI, has_alias, '核磁共振')\n"
    "  → Keep both, as the relation's purpose is to enumerate different names\n"
    "• Synonym relations: 'synonym', 'equivalent_term'\n"
    "• Translation relations: 'translation', 'in_language'\n\n"
    "DO DEDUPLICATE for most other relations where tails refer to actual entities:\n"
    "• Factual relations: 'located_in', 'part_of', 'causes', 'treats'\n"
    "  Example: (Hospital, located_in, '北京市') + (Hospital, located_in, '北京')\n"
    "  → Merge to '北京市', as they refer to the same location\n\n"
    ...
    "✗ Listed together: \"X, Y, Z are enumerated together\" → enumeration ≠ coreference\n"
    "✗ Part-whole relationship: \"X and Y are parts of Z\" → being parts of same whole ≠ identity\n"
    ...
    "✗ Collective representation: \"X, Y, Z together form/represent W\" → components ≠ whole\n"
    ...
    "=" * 80 + "\n"
    "STEP 2: REPRESENTATIVE SELECTION (For each group, choose the best expression)\n"
    "=" * 80 + "\n\n"
    "For each equivalence group identified in Step 1, select ONE member as the representative.\n\n"
    "SELECTION PRINCIPLES (apply in priority order):\n"
    "1. INFORMATION COMPLETENESS: Prefer expressions that convey more information\n"
    "   • Full form > abbreviation (e.g., 'Magnetic Resonance Imaging' > 'MRI')\n"
    "   • Complete name > partial name (e.g., 'Beijing Municipality' > 'Beijing')\n\n"
    "2. SELF-EXPLANATORY: Prefer expressions understandable without domain knowledge\n"
    "   • Descriptive term > technical code (e.g., 'hypertension' > 'I10')\n"
    "   • Common name > chemical formula (e.g., 'water' > 'H₂O' in general contexts)\n\n"
    "3. CANONICALITY: Prefer the standard/official form in the domain\n"
    "   • Official name > informal name\n"
    "   • Current name > deprecated name\n\n"
    "4. UNAMBIGUITY: Prefer expressions with less ambiguity\n"
    "   • Specific term > general term (e.g., 'acetylsalicylic acid' > 'aspirin' in medical contexts)\n\n"
    "5. When principles conflict, prioritize based on the knowledge graph's intended audience\n\n"
    "PROHIBITED SELECTION CRITERIA (Do NOT use these to select representative):\n"
    "✗ FORM-BASED: length, brevity, character count → Information completeness > brevity\n"
    "✗ POSITION-BASED: alphabetical order, index position → Arbitrary ordering is irrelevant\n"
    "✗ FREQUENCY-BASED: how often it appears → Descriptiveness > frequency\n"
    "✗ SUBJECTIVE: familiarity, personal preference → Use objective principles\n\n"
    "CORE PRINCIPLE: Select based on SEMANTIC and INFORMATIONAL value, not form or frequency.\n\n"
    "=" * 80 + "\n"
    "OUTPUT REQUIREMENTS:\n"
    "=" * 80 + "\n\n"
    ...
    "   a) Why group members are coreferent (Step 1 reasoning)\n"
    "   b) Why the representative was chosen (Step 2 reasoning)\n\n"
...
DEFAULT_ATTRIBUTE_DEDUP_PROMPT = (
    ...
    "• Synonym relations: 'synonym', 'equivalent_term'\n"
    "• Translation relations: 'translation', 'in_language'\n\n"
    "DO DEDUPLICATE for most attribute relations:\n"
    "• Factual attributes: 'has_value', 'measurement', 'property'\n\n"
    "FUNDAMENTAL PRINCIPLE:\n"
    ...
    "✗ Listed together: \"V1, V2, V3 are enumerated together\" → enumeration ≠ equivalence\n"
    "✗ Part-whole relationship: \"V1 and V2 are parts/aspects of V\" → being parts ≠ being same\n"
    ...
    "✗ Set membership: \"V1, V2 ∈ {set of attributes}\" → being in same set ≠ being same element\n\n"
    ...
    "3. Choose representative by applying these principles (in priority order):\n"
    "   a) INFORMATION COMPLETENESS: Prefer expressions that convey more information\n"
    "      • Full form > abbreviation (e.g., 'Magnetic Resonance Imaging' > 'MRI')\n"
    "      • Complete specification > partial (e.g., '10 mg/day' > '10 mg')\n"
    "   b) SELF-EXPLANATORY: Prefer expressions understandable without domain knowledge\n"
    "      • Descriptive term > technical code (e.g., 'high blood pressure' > 'HTN')\n"
    "      • Common name > chemical formula (e.g., 'water' > 'H₂O' in general contexts)\n"
    "   c) CANONICALITY: Prefer the standard/official form in the domain\n"
    "      • Standard unit > non-standard unit\n"
    "      • Official terminology > colloquial terminology\n"
    "   d) PRECISION: Prefer expressions with appropriate precision level\n"
    "      • Exact value > approximate value\n"
    "      • Specific measurement > general description\n"
    "   e) When principles conflict, prioritize based on the knowledge graph's intended audience\n\n"
    "PROHIBITED SELECTION CRITERIA - Do NOT base representative selection on:\n"
    "   ✗ FORM-BASED features (surface characteristics unrelated to meaning):\n"
    "     Examples: length, brevity, compactness, character count\n"
    "   ✗ POSITION-BASED features (arbitrary ordering or placement):\n"
    "     Examples: alphabetical order, index position, appearance order in text\n"
    "   ✗ FREQUENCY-BASED features (statistical occurrence patterns):\n"
    "     Examples: how often it appears, which one is mentioned first/more\n"
    "   ✗ SUBJECTIVE features (personal or model-specific preferences):\n"
    "     Examples: familiarity, ease of processing, personal preference\n\n"
    "GENERAL PRINCIPLE: Representative selection must be based on SEMANTIC and INFORMATIONAL criteria,\n"
    "not on arbitrary, statistical, or form-based features. Choose the expression that conveys the\n"
    "most complete and accessible information, regardless of length or frequency.\n\n"
    "4. Provide clear rationale based on VALUE IDENTITY and representative selection principles\n\n"
    "Respond with strict JSON using this schema:\n"
    "{{\n"
    "  \"groups\": [\n"
...
DEFAULT_SEMANTIC_DEDUP_PROMPT_ZH = (
    ...
    "关键要求：依赖关系类型的去重\n"
    "去重行为取决于关系类型：\n\n"
    "不要去重 - 如果关系本身就是用来收集多个表达的：\n"
    "• 别名/异名关系：'别名', 'has_alias', 'also_known_as', '异名'\n"
    "  示例：(MRI, 别名, '磁共振成像') + (MRI, 别名, '核磁共振')\n"
    "  → 保留两者，因为关系的目的就是枚举不同的名称\n"
    "• 同义词关系：'同义词', 'synonym', '等价术语'\n"
    "• 翻译关系：'翻译', 'translation', '其他语言表达'\n\n"
    "应该去重 - 对于大多数指向实际实体的关系：\n"
    "• 事实关系：'位于', 'part_of', '导致', '治疗'\n"
...
DEFAULT_ATTRIBUTE_DEDUP_PROMPT_ZH = (
    ...
    "✗ 并列列举：\"V1、V2、V3被一起列举\" → 列举 ≠ 等价\n"
    "✗ 部分-整体关系：\"V1和V2是V的组成部分/方面\" → 是组成部分 ≠ 相同\n"
    "✗ 相关属性：\"属性_A影响属性_B\" → 关系 ≠ 等价\n"
    "✗ 相似量级：\"值_1 ≈ 值_2\" → 相似性 ≠ 同一性\n"
    "✗ 共现：\"V1和V2一起出现\" → 相关性 ≠ 等价\n"
    "✗ 集合成员资格：\"V1, V2 ∈ {属性集合}\" → 在同一集合中 ≠ 是同一元素\n\n"
    "多值属性：\n"
