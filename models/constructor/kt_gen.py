...
DEFAULT_SEMANTIC_DEDUP_PROMPT = (
    ...
    "- COREFERENCE (merge): Two expressions denote the SAME entity/concept\n"
    "  ✓ 'NYC' and 'New York City' → coreferent (same city)\n"
    "  ✓ 'H2O' and '水' → coreferent (same substance)\n"
    "  ✓ '10 cm' and '100 mm' → coreferent (same measurement)\n\n"
    "- SEMANTIC RELATEDNESS (do NOT merge): Two expressions are related but denote DIFFERENT entities/concepts\n"
    "  ✗ 'cause' and 'effect' → related but different\n"
    "  ✗ 'parent concept' and 'child concept' → related but different\n"
    "  ✗ 'dimension A' and 'dimension B' of the same phenomenon → related but different\n"
    "  ✗ 'spatial resolution' and 'edge sharpness' → related image quality metrics but different concepts\n\n"
    "GUIDING PRINCIPLES:\n"
    "1. SUBSTITUTION TEST: Merge only if the two tails can be substituted for each other in all contexts \n"
    "   without changing meaning. If substitution changes the information conveyed, keep them separate.\n\n"
    "2. FOCUS ON DENOTATION: Judge based on what each tail directly refers to, not on associations, \n"
    "   implications, causes, effects, or broader semantic fields.\n\n"
    "3. ONE CONCEPT = ONE GROUP: If two tails describe distinct concepts (even if closely related), \n"
    "   they belong to different groups. Do NOT merge based on:\n"
    "   - Causal relationships (A causes B)\n"
    "   - Part-whole relationships (A is part of B)\n"
    "   - Taxonomic relationships (A is a type of B)\n"
    "   - Co-occurrence or correlation\n"
    "   - Shared domain or context\n\n"
    "4. CONSERVATIVE MERGING: When uncertain whether two tails are truly coreferent, keep them separate. \n"
    "   False splits are less harmful than false merges in knowledge graphs.\n\n"
    "5. Every input index must appear in exactly one group.\n"
    "6. Choose the most informative and complete expression as the representative.\n\n"
    "Respond with strict JSON using this schema:\n"
    "{{\n"
    "  \"groups\": [\n"
    "    {{\"members\": [1, 3], \"representative\": 3, \"rationale\": \"Why these tails are coreferent.\"}}\n"
    "  ]\n"
    "}}\n"
)
...
DEFAULT_ATTRIBUTE_DEDUP_PROMPT = (
    ...
    "CRITICAL DISTINCTION:\n"
    "- EQUIVALENCE (merge): Two values denote the SAME property with the SAME measurement/state\n"
    "  ✓ '10 cm' and '100 mm' → equivalent (same length)\n"
    "  ✓ 'red' and 'color: red' → equivalent (same color value)\n"
    "  ✓ 'temperature 37°C' and 'temperature 98.6°F' → equivalent (same temperature)\n\n"
    "- NON-EQUIVALENCE (do NOT merge): Two values describe different properties or different values\n"
    "  ✗ 'length 10 cm' and 'width 10 cm' → different properties\n"
    "  ✗ 'temperature 37°C' and 'temperature 38°C' → different values\n"
    "  ✗ 'spatial resolution' and 'edge sharpness' → different quality dimensions\n"
    "  ✗ 'cause X' and 'effect Y' → causally related but distinct\n\n"
    "GUIDING PRINCIPLES:\n"
    "1. EQUIVALENCE TEST: Merge only if the two values represent exactly the same property-value pair. \n"
    "   Different properties, different measurements, or different states must remain separate.\n\n"
    "2. DIRECT DENOTATION: Focus on what each value directly describes. Do NOT merge based on:\n"
    "   - Causal relationships\n"
    "   - Correlation or co-occurrence\n"
    "   - Hierarchical relationships (general vs specific)\n"
    "   - Different dimensions/aspects of a phenomenon\n"
    "   - Shared domain or context\n\n"
    "3. UNIT CONVERSION ONLY: The only acceptable transformations for merging are:\n"
    "   - Unit conversions (e.g., cm ↔ mm)\n"
    ...
    "4. CONSERVATIVE MERGING: When uncertain, keep them separate. In technical and scientific domains, \n"
    "   different terms usually denote different concepts, even if related.\n\n"
    "5. Every input index must appear in exactly one group.\n"
    "6. Choose the most complete and informative description as the representative.\n\n"
    "Respond with strict JSON using this schema:\n"
    "{{\n"
    "  \"groups\": [\n"
    "    {{\"members\": [1, 3], \"representative\": 3, \"rationale\": \"Why these values are equivalent.\"}}\n"
    "  ]\n"
    "}}\n"
)
